...
(** Trickier than assignments to partially evaluate is the
    conditional, [TEST b1 THEN c1 ELSE c2 FI].  If [b1] simplifies to
...
Inductive pe_com : com -> pe_state -> com -> pe_state -> com -> Prop :=
  (* | BLA : forall pe_st l a1, SKIP / pe_st \\ (l ::= a1) / pe_st / SKIP *)
  | PE_Skip : forall pe_st,
...
Proof. intros c pe_st pe_st' c' c'' Hpe.
  ...
  try (inversion Heval; subst;
       try (rew_pe_bexp H; solve_by_invert);
       []);
  eauto.
  - (* PE_AssStatic *) econstructor. econstructor.
    rewrite -> pe_aexp_correct. rewrite <- pe_update_update_add.
...
Theorem pe_com_sound:
  ...
  (c' / pe_st' / c'' / st \\ st'' # n) ->
  (pe_update st pe_st =[ c ]=> st'').
Unset Printing Notations. Proof. intros c pe_st pe_st' c' c'' Hpe.
  induction Hpe;
...
From PLF Require Import Mtac2Tactics.
Import Mtac2Tactics.notations.

Theorem pe_com_complete_mtac2: 
  forall c pe_st pe_st' c' c'', c / pe_st \\ c' / pe_st' / c'' ->
  forall st st'' n,
  (c / pe_update st pe_st \\ st'' # n) ->
  (c' / pe_st' / c'' / st \\ st'' # n).
...
Local Definition work_pe_bexp :=
       select_and (fun a b c=>beval a b = c) (fun _ x=>
       nx <- M.pretty_print x;
       rewrite1r_in pe_bexp_correct x &>
       select_and (fun a b c=>pe_bexp a b = c) (fun _ H=>
       x' <- M.get_var nx;
       dcase x' as v in rewrite1r_in H v)).

Local Definition inv :=
  intros_until ceval_count &> inversion1 &> subst &> eauto.

Local Ltac solve_pe_bexp :=
    ▷ work_pe_bexp;
    solve_by_invert.

Local Ltac edestr T :=
  let n := fresh in
  ▷ (name_it T (fun n:unit=>tt));
  edestruct n; first eassumption;
  clear n.

  ▷ (intros_until pe_com).
  ▷ (induction1 &> (case PE_Skip, PE_WhileFixedEnd do (inv &> has_goals 0)) &>
               T.try (inv &> has_goals 1)).
  - (* PE_AssStatic *)
    ▷ (check_goal (fun a b c d=>SKIP / a / SKIP / b \\ c # d)).
    econstructor.
    + by econstructor.
    + rewrite pe_aexp_correct -pe_update_update_add.
      ▷ (T.select (pe_aexp _ _ = _) >>= rewrite1r).
      by apply: E'Skip.
    + by [].
  - (* PE_AssDynamic *)
    econstructor.
    + by econstructor.
    + rewrite pe_aexp_correct -pe_update_update_remove.
      by apply: E'Skip.
    + by [].
  - (* PE_Seq *)
    edestr (fun a b c d e=> a / b / SKIP / c \\ d # e).
    ▷ (T.select (SKIP / _ \\ _ # _) >>= inversion). subst.
    edestr (fun a b c d e => a / b / c'' / c \\ d # e).
    econstructor; eauto. omega.
  ...
  - (* PE_If *)
    ▷ inv. 
    + (* E'IfTrue *)
      edestr (fun a b c d e => c1' / a / b / c \\ d # e).
      econstructor.
      * apply: E_IfTrue; first by rewrite -pe_bexp_correct.
        apply: E_Seq; first by eassumption.
        by apply: eval_assign.
      * rewrite -assign_removes; eassumption.
      * assumption.

    + (* E_IfFalse *)
      edestr (fun a b c d e => c2' / a / b / c \\ d # e).
      econstructor.
      * apply: E_IfFalse; first by rewrite -pe_bexp_correct.
        apply: E_Seq; first by eassumption.
        by apply: eval_assign.
      * rewrite pe_compare_update.
        rewrite -assign_removes; first by eassumption.
      * assumption.
  ...
  - (* PE_WhileTrue *)
    ...
    + edestr (fun a b c d e=> a / b / SKIP / c \\ d # e).
      ▷ (T.select (SKIP / _ \\ _ # _) >>= inversion). subst.
      edestr (fun a b c d e => a / b / c2'' / c \\ d # e).
      econstructor; eauto. omega.
  ...
  - (* PE_While *) 
    ...
    + (* E_WhileFalse *) 
      ...
      * rewrite -assign_removes.
        by ▷(T.select (_ = SKIP%imp \/ _) >>= inversion); subst.
      * by [].
    ...
    + (* E_WhileTrue *)
      ...
      econstructor.
      * apply: E_IfTrue; first by rewrite -pe_bexp_correct.
        repeat apply: E_Seq; eauto. by apply: eval_assign.
      * rewrite pe_compare_update -assign_removes. eassumption.
      * omega.
  ...
  - (* PE_WhileFixedLoop *)
    ...
    + exfalso.
      generalize dependent (S (n1 + n2)). intros n.
      ▷(H1 <- T.select (pe_bexp _ _ = _);
        H2 <- T.select (pe_compare _ _ = _);
        select_and (fun a b c d e=> a / b / SKIP / c \\ d # e) (fun _ IH1=>
        select_and (fun a b c d e=> c2' / a / b / c \\ d # e) (fun _ IH2=>
        clear_except [m: Dyn H1 | Dyn H2 | Dyn IH1 | Dyn IH2]))).
      generalize dependent st.
      induction n using lt_wf_ind; intros st. (▷ inversion1); subst.
      * (* E'WhileFalse *)
        ▷ (work_pe_bexp).
        by ▷ (T.select (beval _ _ = _) >>= inversion).
      
      * (* E'WhileTrue *)
        edestr (fun a b c d e=> a / b / SKIP / c \\ d # e).
        ▷ (T.select (SKIP / _ \\ _ # _) >>= inversion). subst.
        edestr (fun a b c d e f g=> a / b / (WHILE c DO d END) / e \\ f # g).
        ▷ (H1 <- T.select (pe_compare _ _ = _);
           H2 <- T.select ((WHILE _ DO _ END) / _ \\ _ # n');
           trewrite_in LeftRewrite (pe_compare_nil_update _ _ H1) [m: Dyn H2]).
        by ▷ (H <- T.select ((WHILE _ DO _ END) / _ \\ _ # n');
              select_and False (fun _ H0=>
              tapply_in H0 H &> [m: T.idtac | tomega])).

  - (* PE_WhileFixed *) 
    intros st st'' n. generalize dependent st.
    induction n using lt_wf_ind; intros st;
      ▷ (inversion1 &> subst &> select_and (fun a b c=>beval a b = c) (fun _ =>rewrite1r_in pe_bexp_correct)).
    + (* E'WhileFalse *) by eauto.
    + (* E'WhileTrue *)
      edestr (fun b c d e=> c1' / b / SKIP / c \\ d # e).
      ▷ (T.select (SKIP / _ \\ _ # _) >>= inversion). subst.
      edestr (fun a b c d e f g=> a / b / (WHILE c DO d END) / e \\ f # g).
      ▷ (H3 <- T.select (pe_compare _ _ = _);
         H <- T.select (_ / pe_update st' _ \\ _ # _);
         select_and (fun a b c d e f => (WHILE a DO b END) / c / SKIP / d \\ e # f) (fun _ H4=>
         nH <- M.pretty_print H;
         rewrite1l_in (pe_compare_nil_update _ _ H3) H &>
         H' <- M.get_var nH;
         dcase H' as v in
         tapply_in H4 v &> [m: T.idtac | tomega] &>
         H' <- M.get_var nH;
         dcase H' as v in
         inversion v)).
      econstructor; [ apply: E_WhileTrue; eauto | eassumption | omega].
Qed.
